...
**Известное ограничение:** `git commit -am` в `save_tasks()` коммитит через `-a`, то есть подхватывает вообще все изменённые tracked-файлы репозитория, не только `tasks.json`. Если в этот момент `tools/today_server.py` тоже модифицирован (например, во время разработки самого дашборда) — commit-хук блокирует коммит (правило "изменены tools/*.py, но не обновлён tools-manual.md"), и `subprocess.run(..., capture_output=True)` эту ошибку проглатывает молча. Практический эффект: пока идёт правка `today_server.py`, автокоммит из дашборда тихо не срабатывает вообще (задачи в tasks.json продолжают меняться на диске корректно, просто без промежуточных коммитов) — накопившиеся изменения нужно закоммитить вручную после того как код-правка сама закоммичена.

**Карточка задачи (модалка):** маркер (`🔴/🟢/🟧`) кликабелен и меняется по клику (`makeMarkerField`, эндпоинт `/set-marker`, общий для всех view). Поле "Таймер" — кнопка ▶ Старт/⏸ Стоп, пишет `timer_started_at` (ISO) при старте и накапливает `time_spent_min` при стопе (`/start-timer`, `/stop-timer`). Список областей (`window.AREAS`) выводится в общий `make_page()`, доступен одинаково на "Сегодня" и "Задачи" — раньше был только на "Задачи", из-за чего на "Сегодня" в карточке не было выбора области. `_render_scheduled_item` (раздел "Сегодня") теперь тоже рендерит кнопку маркера, как и `render_task_row` (раздел "Задачи"). Фоновый polling (`/poll`, каждые 2 сек) не перезагружает страницу, пока открыта карточка задачи (`#task-modal.open`) — иначе запущенный таймер или несохранённые правки в модалке сбрасывались бы каждым чужим изменением файла.

**Activity log:** каждое мутирующее действие в дашборде (toggle-task, set-marker, set-parent, start/stop-timer, delete, rename, add-task и т.д.) логируется одной JSONL-строкой в `04_THINKING/activity-log/YYYY-MM-DD.jsonl` (`log_activity()`, единая точка в `do_POST`, вызывается после диспетчеризации). Формат строки: `{"ts", "action" (путь без слэша), "id", "title", "payload"}`. Не коммитится автоматически (не обёрнуто в `save_tasks()`), файлы копятся локально — цель: сырой фид активности по дашборду для восстановления картины дня, не требует отдельного парсинга tasks.json diff.
